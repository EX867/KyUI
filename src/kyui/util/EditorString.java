...
public class EditorString {//editorString is based on line.
  ...
  public synchronized String getLine(int line_) {
    return l.get(line_);
  }
  ...
  public synchronized void addLine(int line_, String text) {
    if (text == null) return;
    l.add(line_, text);
  }
  public synchronized void deleteLine(int line_) {
    l.remove(line_);
  }
  public synchronized void setLine(int line_, String text) {
    if (text == null) return;
    l.set(line_, text);
  }
  ...
  public void selectionRight(boolean word) {
    if (point == l.get(l.size() - 1).length() && line == l.size() - 1) return;
    if (hasSelection() == false) {
      resetSelection();
    }
  ...
  public String getSelectionPart(int line_) {
    if (line_ < selStartLine || line_ > selEndLine) {
      return "";
    } else if (line_ > selStartLine && line_ < selEndLine) {
      return getLine(line_) + "\n";
    } else if (line_ == selStartLine) {
      if (selStartPoint > getLine(line_).length() || selStartPoint < 0) return "";//FIX>>????????????????????????????????
      if (selStartLine == selEndLine) {
        if (selEndPoint > getLine(line_).length() || selEndPoint < 0) return "";//FIX>>????????????????????????????????
    ...
    } else if (line_ == selEndLine) {
      if (selEndPoint > getLine(line_).length() || selEndPoint < 0) return "";//FIX>>????????????????????????????????
      return getLine(selEndLine).substring(0, selEndPoint);
    }
    return "";//unreachable
  }
  ...
  //===Privates===//
  protected boolean isSpaceChar(char in) {
    if (in == ' ' || in == '\t' || in == '\n' || in == '\r') return true;
    return false;
  }
